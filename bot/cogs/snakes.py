...
class Snakes:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            if isinstance(url, list):
                if kwargs.get('autocorrect', False):
                    return await self.get_snek(url[0], kwargs)
            ...
            return {
                "name": content["title"],
                "info": content.get("extract", "I don't know about that snake!") if len(
                    kwargs.get('details', ())) == 0 else "Requested details:" + str(kwargs['details']),
                "image": image
            }
    ...
    async def snake_card(self, ctx: Context, *args):
        content = await self.get_snek(args[0] if args else None,
                                      self.kwargs(args[1:], positional_args=[("autocorrect", False), ("details", ())]))
