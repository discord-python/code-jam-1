...
class Snakes:
    ...
    def kwargs(*args):
        return {
            k: ast.literal_eval(v) for k,v in dict(arg.split('=') for arg in args)
        }
    ...
    async def get_snek(self, name: str = None, autocorrect: bool= False, details: list = None, kwargs: dict = None) -> Dict[str, str]:
        """If a name is provided, this gets a specific snake. Otherwise, it gets a random snake."""

        details = details or kwargs['details']
        ...
        async with aiohttp.ClientSession() as session:
            ...
            if isinstance(url, list):
                if kwargs.get("autocorrect") or autocorrect:
    ...
    async def get(self, ctx: Context, name: str = None, autocorrect: str = None, details: str = None):
        ...
        content = await self.get_snek(
            name,
            ast.literal_eval(autocorrect.capitalize()),
            details,
            self.kwargs(autocorrect, details)
        )
